...
`linalg.Matmul(a, b, into: c)` over views, with a **fused epilogue**: the
work done on each output tile before it is written. It is described as
data, `.Bias(v)`, `.Scale(s)`, `.Activation(.Gelu)`, `.Residual(r)` and
`.Cast(.bfloat16)`, in the style of cuBLASLt and CUTLASS epilogues and
Triton fusion. Each combination is specialized into its own image; no
